...
```sh
npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 cache inspect --json --project-root .
npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 remote clean --json --scope results
```
...
1. Preview one scope: `npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 cache clean --json --project-root . --scope local-generated` or `local-client-cache`.
2. Show the exact count, bytes, paths, expiry, and `planId` to the user.
3. Apply only after explicit confirmation: `npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 cache clean --json --project-root . --plan-id PLAN_ID --yes`.
...
1. Preview with `npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 remote clean --json --scope results|snapshot|project`.
2. Report the returned `report`, `planId`, and `expiresAt`. Explain that `project` is the broadest scope.
3. Apply only after explicit user confirmation: `npx --yes --ignore-scripts remote-latexmk@0.3.0-rc.3 remote clean --json --plan-id PLAN_ID --yes`. Do not pass `--scope` during apply.
