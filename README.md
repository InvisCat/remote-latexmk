...
only if the command reports that the installed copy was not updated
automatically. Restart Codex if it was running, then start a new chat from the
paper directory.
...
Quick Start. It updates both the personal marketplace source and the installed
Codex copy when the Codex CLI is available. If it reports that automatic
installation was unavailable, select **Install** or **Update** on the opened
page. Restart Codex, then start a new task. The saved server login is kept.
...
- Fixed Codex Desktop Plugin updates so the installer updates and verifies the
  versioned Codex installed copy when Codex CLI is available, instead of only
  refreshing the personal marketplace source.
- Kept the Plugin page as a fallback when automatic installation is
  unavailable, with explicit output showing whether the installed copy was
  updated.
...
- Fixed Codex Desktop Plugin compilation when the host does not advertise MCP
  workspace roots by safely binding the local MCP server to the task working
  directory.
- Kept MCP roots authoritative when advertised: rejected, invalid, or
  ambiguous roots still fail closed instead of falling back.
- Added Codex-style MCP transcript and packaged-Plugin regression tests.
...
- Fixed native server compilation when TeX Live is installed under the private
  `~/.remote-latexmk` directory. The compile sandbox now keeps the installer's
  explicit TeX toolchain path without inheriting the full server environment.

### remote-latexmk 0.3.0-rc.3

- Replaced manual version edits and temporary recovery workflows with a
  Release PR flow driven by one root package version.
- Made npm a required release channel, verified existing package contents on
